...
function databaseInit() {
    ...
    database.ref().on("value", function(snapshot) {
        ...
        tableMinutesAway.html(snapshot.val().trains[1].minutesAway);
...
function addTrain() {
    ...
    if ($("#trainName").val() == '' || $("#destination").val() == '' || $("#firstTrainTime").val() == '' || $("#frequency").val() == '') {
        ...
        newTrain.firstTrainTime = nextArrival(newTrain.firstTrainTime, newTrain.frequency);

        // Make a new object property called "minutesAway" and assign it using the function I defined below
        newTrain.minutesAway = minutesAway(newTrain.firstTrainTime);

        // Format the firstTrainTime variable to be a pretty string instead of an ugly object
        newTrain.firstTrainTime = newTrain.firstTrainTime.format('h:mm A');
